...
export default function LedgerReportPage() {
  ...
  // Clear dynamic filters if tab is changed
  useEffect(() => {
          ...
          {isLoading ? (
            ...
            visibleEntries.map((item, i) => (
              <tr key={i} className="hover:bg-slate-50/80 transition-colors">
                ...
                <td className="px-5 py-3">
                  <span
                    className={`inline-flex px-2 py-1 bg-slate-100 border border-slate-200 rounded text-xs font-bold ${item.paymentCategory === "CASH" ? "text-emerald-700 bg-emerald-50 border-emerald-100" : "text-blue-700 bg-blue-50 border-blue-100"}`}
                  >
                    {item.mode}
                  </span>
                </td>
