...
import { getLedgerDashboard } from "@/app/actions/reports/id-based-details";
import Link from "next/link";

export default function LedgerIdPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const resolvedParams = use(params);
  const ledgerId = resolvedParams.id;
  ...
  const fetchDashboardData = async () => {
    setIsLoading(true);
    const res:any = await getLedgerDashboard(ledgerId, fromDate, toDate);
  ...
  const filteredTransactions =
    data?.transactions?.filter(
      (tx: any) =>
        ...
        tx.companyName.toLowerCase().includes(tableSearchQuery.toLowerCase()) ||
  ...
  const handleExportCSV = () => {
    ...
    try {
      let csvContent =
        "Voucher No,Date,Type,Origin Company,Particulars,Mode,Debit (In),Credit (Out)\n";

      filteredTransactions.forEach((tx: any) => {
        const inwards = tx.flowType === "in" ? tx.amount : 0;
        const outwards = tx.flowType === "out" ? tx.amount : 0;
        csvContent += `"${tx.voucherNo}","${tx.date}","${tx.type}","${tx.companyName}","${tx.particulars}","${tx.mode || "N/A"}",${inwards},${outwards}\n`;
      });
      ...
      link.setAttribute(
        "download",
        `Ledger_Report_${data.ledgerName.replace(/\s+/g, "_")}_${fromDate}_to_${toDate}.csv`,
      );
    ...
    <div className="p-4 lg:p-8 space-y-6 max-w-7xl mx-auto bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4">
          ...
          <div className="h-14 w-14 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center border border-emerald-100 shadow-sm">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto bg-slate-50 p-2.5 rounded-2xl border border-slate-200">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500"
            />
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 border-y border-r border-l-4 border-slate-200 border-l-slate-700 shadow-sm flex justify-between items-center group hover:shadow-md transition-shadow">
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-900">
              ₹{" "}
              {Math.abs(data?.openingBalance || 0).toLocaleString("en-IN", {
        ...
        <div className="bg-white rounded-2xl p-6 border-y border-r border-l-4 border-slate-200 border-l-blue-600 shadow-sm flex justify-between items-center group hover:shadow-md transition-shadow">
          <div>
            ...
            <h3 className="text-2xl font-black text-blue-700">
              ₹{" "}
              {Math.abs(data?.closingBalance || 0).toLocaleString("en-IN", {
                minimumFractionDigits: 2,
              })}
              <span className="text-xs font-bold text-blue-400 ml-1">
                {data?.closingBalance >= 0 ? "Cr" : "Dr"}
              </span>
            </h3>
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-colors"
              />
            </div>
            <button
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-[11px] tracking-wider border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4">Origin Company</th>
              ...
              {paginatedTransactions.length === 0 ? (
                ...
                paginatedTransactions.map((tx: any) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3.5">
                      <span className="inline-flex px-2.5 py-1 bg-purple-50 border border-purple-100 text-purple-700 rounded-md text-[11px] font-bold uppercase tracking-wider">
                        {tx.companyName}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-3.5 text-center">
                      <button
                        ...
                        className="p-2 text-slate-400 hover:text-emerald-600 bg-white border border-slate-200 hover:border-emerald-300 rounded-lg transition-all shadow-sm group-hover:bg-emerald-50"
                        title="View Details"
                      >
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`h-8 w-8 rounded-lg text-sm font-bold transition-colors ${currentPage === pageNum ? "bg-emerald-50 text-emerald-600 border border-emerald-200" : "text-slate-500 hover:bg-slate-50 border border-transparent"}`}
                    >
                      {pageNum}
                    </button>
                  );
            ...
            <div className="p-8 bg-slate-50/50 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100 border-b border-slate-100">
                  ...
                  <div className="p-5">
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                      <BookOpen className="h-3.5 w-3.5" /> Origin Company
                    </p>
                    <p className="font-semibold text-slate-800 text-base">
